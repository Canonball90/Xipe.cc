...
public class DamageUtils {
	...
	public static float getExplosionDamage(Vec3d explosionPos, float power, LivingEntity target) {
		if (MinecraftClient.getInstance().world.getDifficulty() == Difficulty.PEACEFUL)
			return 0f;

		double maxDist = power * 2;
		if (!MinecraftClient.getInstance().world.getOtherEntities(null, new Box(
			...
			if (distExposure <= 1.0) {
				...
				if (diff != 0.0) {
					...
					if (target instanceof PlayerEntity) {
						if (MinecraftClient.getInstance().world.getDifficulty() == Difficulty.EASY) {
							toDamage = Math.min(toDamage / 2f + 1f, toDamage);
						} else if (MinecraftClient.getInstance().world.getDifficulty() == Difficulty.HARD) {
