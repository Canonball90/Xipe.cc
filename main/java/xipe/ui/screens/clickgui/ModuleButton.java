...
public class ModuleButton {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Original")) {
		...
		else if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("OldSchool")) {// module.isEnabled() ? Color.pink.getRGB() : -1
			this.OLDGUI(matrices, mouseX, mouseY, delta);
		}
			...
			DrawableHelper.fill(matrices, parent.x - 1, parent.y + offset - 1, parent.x + parent.width +1, parent.y + offset + parent.height + 1, new Color(94,255,175).getRGB());
			if (parent.buttons.indexOf(this) == parent.buttons.size() - 1) {
				DrawableHelper.fill(matrices, parent.x - 1, parent.y + offset, parent.x + parent.width + 1, parent.y + offset + parent.height + 1, new Color(94,255,175).getRGB());
				}
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, module.isEnabled() ? new Color(0,0,0,160).getRGB() + Color.darkGray.getRGB() : Color.darkGray.getRGB());
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, module.isEnabled() ? new Color(0,0,0,160).getRGB() + Color.darkGray.getRGB() : Color.darkGray.getRGB());
			if(ModuleManager.INSTANCE.getModule(Gui.class).cFont.isEnabled()) {
				parent.nuitofont.draw(matrices, module.getName(), parent.x-1 + offsetY, parent.y-6 + offset + offsetY, -1, false);
			}else {
			...
			if(ModuleManager.INSTANCE.getModule(Gui.class).outline.isEnabled()) {
				if(ModuleManager.INSTANCE.getModule(Gui.class).rainbow.isEnabled()) {
					...
					DrawableHelper.fill(matrices, parent.x - 1, parent.y + offset, parent.x + parent.width +1, parent.y + offset + parent.height, Color.blue.getRGB());
			}
			...
			if (parent.buttons.indexOf(this) == parent.buttons.size() - 1) {
				if(ModuleManager.INSTANCE.getModule(Gui.class).rainbow.isEnabled()) {
					...
					RenderUtil.renderRoundedQuad(matrices, new Color(rainbow(300)),parent.x - 4, parent.y + offset, parent.x + parent.width +4, parent.y + offset + parent.height + 4, 5, 10);
				}else if(ModuleManager.INSTANCE.getModule(Gui.class).custom.isEnabled()) {
					DrawableHelper.fill(matrices, parent.x - 1, parent.y + offset, parent.x + parent.width +1, parent.y + offset + parent.height + 1, color);
				}else {
					DrawableHelper.fill(matrices, parent.x - 1, parent.y + offset, parent.x + parent.width +1, parent.y + offset + parent.height + 1, Color.blue.getRGB());
			}
				}
			}
			//main
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, module.isEnabled() ? new Color(0,0,0,160).getRGB() + Color.darkGray.getRGB() : Color.gray.getRGB());
		...
		if(isHovered(mouseX, mouseY)) {
			...
			if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Original")) {
			this.OriginalThemeDescription(matrices, mouseX, mouseY, delta);
			}else if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Win98")) {
				this.Win98(matrices, mouseX, mouseY, delta);
			}else if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Test")) {
				this.TestTheme(matrices, mouseX, mouseY, delta);
			}
			
		}else if(isHovered(mouseX, mouseY) && ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Clear")) {
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, new Color(0, 0, 0, 0).getRGB());
		}
	...
	public void OriginalThemeDescription(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Original")) {
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 285, 2, 273, 22, Color.pink.getRGB());
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 283, 4, 275, 20, Color.darkGray.getRGB());
			MinecraftClient.getInstance().textRenderer.draw(matrices, module.getDescription(),280, 8, -1);
			}
	}
	public void TestTheme(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Test")) {
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 285, 2, 273, 22, Color.blue.getRGB());
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 283, 4, 275, 20, Color.gray.getRGB());
			MinecraftClient.getInstance().textRenderer.draw(matrices, module.getDescription(),280, 8, -1);
			}
	}
	public void Win98(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Win98")) {
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 285, 2, 273, 22, Color.blue.getRGB());
			DrawableHelper.fill(matrices, parent.mc.textRenderer.getWidth(module.getDescription()) + 283, 4, 275, 20, Color.lightGray.getRGB());
			MinecraftClient.getInstance().textRenderer.drawWithShadow(matrices, module.getDescription(),280, 8, -1);
			}
	}
		...
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("Original")) {
			...
			if(ModuleManager.INSTANCE.getModule(Gui.class).outline.isEnabled()) {
				if(ModuleManager.INSTANCE.getModule(Gui.class).outline.isEnabled() && ModuleManager.INSTANCE.getModule(Gui.class).custom.isEnabled()) {
					DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width +2, parent.y + offset + parent.height, color);
			...
			DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width +2, parent.y + offset + parent.height, rainbow(300));
				}else {
					DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width +2, parent.y + offset + parent.height, Color.pink.getRGB());
				}
			if(ModuleManager.INSTANCE.getModule(Gui.class).bottomOutline.isEnabled()) {
				if (parent.buttons.indexOf(this) == parent.buttons.size() - 1) {
					if(ModuleManager.INSTANCE.getModule(Gui.class).bottomOutline.isEnabled() && ModuleManager.INSTANCE.getModule(Gui.class).custom.isEnabled()) {
						DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width + 2, parent.y + offset + parent.height + 2, color);
						...
						DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width + 2, parent.y + offset + parent.height + 2, rainbow(300));
					}else {
					DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset, parent.x + parent.width + 2, parent.y + offset + parent.height + 2, Color.pink.getRGB());
					}
					}
				}
			}
	...
	public void OLDGUI(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		int offsetY = ((parent.height / 2) - parent.mc.textRenderer.fontHeight / 2);
		int textOffset = ((parent.height / 2) - parent.mc.textRenderer.fontHeight / 2);
		
		if(ModuleManager.INSTANCE.getModule(Gui.class).theme.isMode("OldSchool")) {// module.isEnabled() ? Color.pink.getRGB() : -1
			DrawableHelper.fill(matrices, parent.x - 2, parent.y + offset - 2, parent.x + parent.width + 2, parent.y + offset + parent.height + 2, Color.gray.getRGB());
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, Color.darkGray.getRGB());
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, module.isEnabled() ? Color.red.getRGB() : Color.darkGray.getRGB());
			DrawableHelper.fill(matrices, parent.x, parent.y + offset, parent.x + parent.width, parent.y + offset + parent.height, module.isEnabled() ? new Color(0,0,0,160).getRGB() + Color.darkGray.getRGB() : Color.darkGray.getRGB());
			if(ModuleManager.INSTANCE.getModule(Gui.class).cFont.isEnabled()) {
				parent.nuitofont.draw(matrices, module.getName(), parent.x-1 + offsetY, parent.y-6 + offset + offsetY, -1, false);
			}else {
